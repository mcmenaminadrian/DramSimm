...
/*
 * DramSimm.groovy - Based on mem-test.c from the original distribution
 *
 ...
 *               
 * Port to groovy copyright (c) 2016 Adrian McMenmin
 */
...
def MAX_FILENAME_LENGTH = 1024
def UNIFORM_DISTRIBUTION = 1
def GAUSSIAN_DISTRIBUTION = 2
def POISSON_DISTRIBUTION = 3
def PI = 3.141592654
def MAX_TRACES = 8


/*
void 	set_cpu_memory_frequency_ratio(def biu, int mem_frequency);
...
class MainDramSim {
	...
	def chipset_delay                      /* latency through chipset, in DRAM ticks */
	def bus_queue_depth
	def biu_delay                          /* latency through BIU, in CPU ticks */
	def address_mapping_policy		/* {burger_map|burger_alt_map|sdram_base_map|intel845g_map} */
	def refresh_issue_policy
	def row_buffer_management_policy	/* {open_page|close_page|perfect_page|auto_page} */
	def biu_debug_flag
	def transaction_debug_flag
	def dram_debug_flag
	def trace_input_flag
	def all_debug_flag     		/* if this is set, all dram related debug flags are turned on */
	def wave_debug_flag    		/* cheesy Text Based waveforms  */
	def cas_wave_debug_flag    		/* cheesy Text Based waveforms  */
	def bundle_debug_flag    		/* Watch Bundles that are issued  */
	def amb_buffer_debug_flag		/* watch amb buffers */
	def debug_tran_id_threshold		/* tran id at which to start debug output */
	def debug_tran_id			/* tran id  which to print out why commands are not being output*/
	def debug_ref_tran_id			/* tran id  which to print out why commands are not being output*/
	def get_next_event_flag = false

	def power_stats_fileout = null
	def common_stats_fileout = null
	def trace_filein = [MAX_TRACES]

	def biu_stats = false
	def biu_slot_stats = false
	def biu_access_dist_stats = false
	def bundle_stats = false
	def	tran_queue_stats = true
	def bank_hit_stats = false
	...
	def arrival_distribution_model
	def fake_access_type
	def fake_rid
	def fake_v_address			/* virtual address */
	def fake_p_address			/* physical address */
	def thread_id
	def spd_filein
...
def dramSimulation = new MainDramSim()